...
- ポップオーバーで Wi-Fi またはバッテリーの行を選択すると、そのページが開きます：近くのネットワークとリンク詳細、またはバッテリーの詳細です。Bluetooth 行はペアリング済みデバイスをその場で一覧表示します — タップで接続や切断 — デバイスがすべて入りきらないときは「展開」コントロールが表示されます。
