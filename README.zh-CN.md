...
- **三合一图标**：电池、Wi-Fi 和音量共用一个图标，放在菜单栏、程序坞或两处都行。播放蓝牙音频时，可以让设备自己的图标接管中间的位置。
- **蓝牙音频**：设备图标取代网络图标，音量指示也会变成蓝色，不用打开面板就能看出声音从哪来。
- **电池**：电量百分比、充电或已接电源、充满预计时间，电量偏低时用颜色提醒。
- **电池详情**：点开电池一行，可以看到适配器功率、剩余时间、电压、电流、循环次数和低电量模式，也能直接跳到电池设置。
- **Wi-Fi**：面板上会显示当前网络名称和信号强度；展开后可以查看附近网络、加入网络，或直接关掉 Wi-Fi。
- **蓝牙面板**：显示已连接和已配对的设备，以及 AirPods 电量。默认关闭。
- **音量**：音量大小、静音状态和当前输出设备，样式可选圆点或圆弧。
- **滚动调节**：可以在面板任意位置滚动调节音量，也可以只在音量控件上滚动；向上滚是变大还是变小同样能选。
- **按自己的习惯调整**：图标大小、图标缩放、圆环粗细、状态颜色，以及面板显示哪些区块、按什么顺序排列。
- **菜单栏、程序坞或两处**：程序坞图标可以跟随系统样式，也可以固定为深色或浅色。
- **原生 macOS 操作**：左键打开面板，右键打开菜单；首次启动会有引导，说明图标每一部分代表什么。
- **连接图标**：有线连接、个人热点或互联网共享，也可以继续显示 Wi-Fi 图标。
- **高效更新**：状态变化由系统事件驱动，另有低频轮询兜底。
- **十二种语言**：跟随系统或手动选择，改完立即生效。
- **登录时启动**：可选，macOS 需要批准时会给出引导。
- **自动更新**：Sparkle 检查已签名的更新源，并在安装前校验每次更新。
