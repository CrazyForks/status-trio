...
- 在弹层中点击 Wi-Fi 或电池行会打开对应页面：Wi-Fi 详情与附近网络、电池详情。蓝牙行则直接在行下列出已配对设备，放不下时用「展开」查看。
...
Status Trio 通过 macOS 公开框架读取系统状态，不使用 App Sandbox，也不需要网络权限，且不包含遥测或分析功能。不会读取也不保存 Wi-Fi 密码，也不会请求钥匙串访问。定位权限为可选项，仅在用户选择显示当前 Wi-Fi 网络名称或打开 Wi-Fi 详情时请求。蓝牙权限仅在显示蓝牙面板时请求，用于显示已配对设备的连接状态。
