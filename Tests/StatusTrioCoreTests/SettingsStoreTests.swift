...
final class SettingsStoreTests: XCTestCase {
    ...
    func testPopupSectionOrderDefaultsToBatteryNetworkBluetoothVolumeAudioInput() {
        let store = SettingsStore(defaults: makeSuite().defaults)

        XCTAssertEqual(
            store.popupSectionOrder,
            [.battery, .network, .bluetooth, .volume, .audioInput]
        )
    }
    ...
    func testStoredPopupSectionOrderIsSanitizedAndCompleted() {
        ...
        XCTAssertEqual(
            store.popupSectionOrder,
            [.volume, .network, .battery, .bluetooth, .audioInput]
        )
    }
