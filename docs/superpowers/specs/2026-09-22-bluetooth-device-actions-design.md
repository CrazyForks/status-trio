...
实现落地时有三处偏离本文，均已接受并随分支交付，记录在此以免只读本文的读者被误导：
...
3. **确认态由「视图内 `@State`」改为 controller 持有、在面板关闭时清除。** 真机验证发现：从摘要面板关闭弹层时，`StatusBarController` 会刻意保留内容视图 controller（`popoverContentReleaseDelay = 60` 秒）以便快速重开，视图并未离开层级，因此**该视图的 `onDisappear` 根本不会触发**，确认提示会在下次打开时仍挂着——与本文「面板关闭即视为取消」的要求相冲突。改为 `BluetoothDeviceController.pendingDisconnectConfirmation` 持有，并在 `SystemStatusStore.setPopoverVisible(false)` 里清除（与收起电池详情、释放 surface 认领同一处）；这样该规则终于可以被单测钉住（`testPopoverClosingCancelsAPendingBluetoothConfirmation`）。
4. **蓝牙详情页已删除。** 面板里的内联列表加上「展开」控制已经覆盖了同一信息，详情页只剩重复，因此整页移除：摘要行不再是导航按钮（`onOpenDetails` 与 `>` 提示一并去掉），它独有且仍需要的两点搬到行上——齿轮旁的**手动刷新**按钮，以及电量读取失败时**列表下方的一行提示**；`SystemStatusStore` 的 `isBluetoothDetailsOpen` / `openBluetoothDetails()` / `closeBluetoothDetails()` 与 `StatusPopoverView` 的 `.bluetooth` 面板分支随之删除。`BluetoothBatteryLevelHandoffTests` 的第二个界面改用替身，继续守住「认领只能被自己释放」这条回归。
5. **行内不再写出「已连接/未连接」。** 改为音量输出列表的选中态：图标加**强调色圆圈底**、名称**半粗体**、右侧**对勾**；状态词从可见文本里去掉，但保留在行的**无障碍 value** 上（颜色与对勾都不是唯一指示）。「进行中 / 失败」仍然写成文字，由 `BluetoothDeviceRowStatus.drawsText` 这条纯函数规则钉住。
